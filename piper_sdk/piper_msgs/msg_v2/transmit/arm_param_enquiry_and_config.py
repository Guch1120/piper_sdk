...
class ArmMsgParamEnquiryAndConfig:
    ...
    Args:
        param_enquiry: パラメータクエリ
        param_setting: パラメータ設定
        data_feedback_0x48x: 0x48Xメッセージフィードバック設定
        end_load_param_setting_effective: 先端負荷パラメータ設定が有効かどうか
        set_end_load: 先端負荷を設定
    
    ビット記述:

        Byte 0: uint8,パラメータクエリ,先端 V/accをクエリ
                0x01,先端 V/accをクエリ
                0x02,衝突保護レベルをクエリ
                0x03,現在の軌道インデックスをクエリ
                0x04,グリッパー/ティーチングペンダントパラメータインデックスをクエリ ---- V1.5-2バージョン以降
        Byte 1: uint8,パラメータ設定,
                先端 V/acc パラメータを初期値に設定: 0x01
                全関節制限、関節最大速度、関節加速度をデフォルト値に設定: 0x02
        Byte 2: uint8,0x48X メッセージフィードバック設定,
                無効:0x00
                周期フィードバックをオフ: 0x01;
                周期フィードバックをオン: 0x02;
                オンにすると、1〜6番目の関節の現在の先端速度/加速度を周期的に報告
        Byte 3: uint8,先端負荷パラメータ設定が有効かどうか,有効値 : 0xAE
        Byte 4: uint8,先端負荷を設定,
                0x00 : 無負荷;
                0x01 : 半負荷;
                0x02 : 全負荷;
                0x03 : 無効
